...
const LocationModal: React.FC<LocationModalProps> = ({ isOpen, onClose, onSelectLocation }) => {
  ...
  const [justRequestedLocation, setJustRequestedLocation] = useState(false);
  ...
  useEffect(() => {
    if (locationState.permission === 'granted' && locationState.latitude && locationState.longitude) {
        ...
        // If user just requested location, auto-select and close
        if (justRequestedLocation && isOpen) {
          onSelectLocation(location);
          onClose();
          setJustRequestedLocation(false);
        }
    }
  }, [locationState, justRequestedLocation, isOpen, onSelectLocation, onClose]); // Dependency on locationState and justRequestedLocation
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (locationState.permission === 'denied') {
                    alert('Location permission is denied. Please enable it in your browser settings to use current location.');
                    return;
                  }

                  // Always call requestLocation for 'prompt' or 'granted' to trigger browser popup or get location
                  setJustRequestedLocation(true);
                  requestLocation();
                }}
              >
