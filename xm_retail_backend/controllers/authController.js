...
export const saveRegistration = async (req, res) => {
  try {
    const { firstName, lastName, phone, pincode, email, userId } = req.body;

    console.log('Registration data received:', { firstName, lastName, phone, pincode, email, userId });

    // Validate required fields
    if (!firstName || !lastName || !userId) {
      return res.status(400).json({ 
        success: false,
        message: "First name, last name and user ID are required",
        details: {
